...
The methodology `{simbased}` relies on is described in @kingMakingMostStatistical2000. Similar functionality exists in the `clarify` package in Stata [@tomzClarifySoftwareInterpreting2003] and used to be available in the `Zelig` R package [@imaiCommonFrameworkStatistical2008a] . `{simbased}` provides additional flexibility by allowing the user to request any derived quantity, in addition to providing shortcuts for common quantities, including predicted vales at representative values, average marginal effects, and average dose-response functions (described below). `{simbased}` relies on and can be seen as a companion to the `{marginaleffects}` package, which offers similar functionality but uses the delta method for its calculations.

## Using `{simbased}`

There are four steps to using `{simbased}`:
...
If your model is not supported by `{simbased}`, you can omit the `fit` argument and just specify the `vcov` and `coefs` argument, which will draw the coefficients from the distribution named in `dist` (`"normal"` by default).
...
There are several functions in `{simbased}` that serve as convenience wrappers for `sim_apply()` to automate some common derived quantities of interest. These include
...
`sim_apply()` can be used to compute the simulated sampling distribution for any arbitrary derived quantity of interest, but there are some quantities that are common in applied research and may otherwise be somewhat challenging to program on their own, so `{simabsed}` provides shortcut functions to make computing these quantities simple. These functions include `sim_setx()`, `sim_ame()`, and `sim_adrf()`. Each of these can only be used when regression models compatible with `{simbased}` are supplied to the original call to `sim()`.
...
`cbind()` is another generic R function that is typically used to combine two or more datasets columnwise (i.e., to widen a dataset). In `simbased`, `cbind()` can be used to combine two `simbased_est` objects so that the estimates can be examined jointly and so that it is possible to compare them directly. For example, let's say we computed AMEs in two subgroups and wanted to compare them. To do so, we would call `sim_ame()` twice, one for each subset:
...
# Using `{simbased}` with multiply imputed data
...
The arguments to `simmi()` are as follows:

``` r
simmi(fitlist = , n = , vcov = , coefs = , dist = )
```
...
Below we illustrate using `simmi()` and `sim_apply()` with multiply imputed data. We'll use the `africa` dataset from the `{Amelia}` package.
