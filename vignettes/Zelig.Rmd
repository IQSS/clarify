...
`{clarify}` does not combine coefficients, unlike `zelig()`; instead, the models should be fit using `Amelia::with()` and combined using `Amelia::mi.combine()`.
...
Derived quantities can be computed using `clarify::misim()` and `sim_apply()` or its wrappers:
