...
`{clarify}` implements simulation-based inference as an alternative to
...
is computationally intensive since the original model needs to be fit
...
effects. The primary functions of `{clarify}` are `sim()`,
...
predicted values at typical values of the covariates and differences
between them, mirroring `Zelig::setx()` and `Zelig::setx1()`.
...
`{clarify}` provides a shortcut for computing average marginal effects
