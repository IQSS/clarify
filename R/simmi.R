...
#'
#' @description `simmi()` simulates model parameters from multivariate normal or t distributions after multiple imputation that are then used by [sim_apply()] to calculate quantities of interest.
#'
...
#'
#' @return
#' A `simbased_simmi` object, which inherits from `simbased_sim` and has the following components:
...
#' The `"dist"` attribute contains `"normal"` if the coefficients were sampled from a multivariate normal distribution and `"t({df})"` if sampled from a multivariate t distribution. The `"simbased_hash"` attribute contains a unique hash generated by [rlang::hash()].
#'
#' @details
#' `simmi()` essentially combines multiple `sim()` calls applied to a list of model fits, each fit in an imputed dataset, into a single combined pool of simulated coefficients. When simulation-based inference is to be used with multiply imputed data, many imputations are required; see Zhou and Reiter (2010).
#'
#' @references
#'
...
simmi <- function(fitlist,
  ...
  class(out) <- c("simbased_simmi", "simbased_sim")

  out
}

#' @export
print.simbased_simmi <- function(x, ...) {
