...
#'
#' @return
#' A `simbased_misim` object, which inherits from `simbased_sim` and has the following components:
...
misim <- function(fitlist,
  ...
  class(out) <- c("simbased_misim", "simbased_sim")

  out
}

#' @export
print.simbased_misim <- function(x, ...) {
  obj <- deparse1(substitute(x))
  cat("A `simbased_misim` object\n")
