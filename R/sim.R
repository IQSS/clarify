...
#' @param dist a string containing the name of the multivariate distribution to use to draw simulated coefficients. Should be one of `"normal"` (multivariate normal distribution) or `"t_{#}"` (multivariate t distribution), where `{#}` corresponds to the desired degrees of freedom (e.g., `"t_100"`). If `NULL`, the right distribution to use will be determined based on heuristics; see Details.
#'
#' @return
...
#' The `"dist"` attribute contains `"normal"` if the coefficients were sampled from a multivariate normal distribution and `"t"` if sampled from a multivariate t distribution. The `"simbased_hash"` attribute contains a unique hash generated by [rlang::hash()].
#'
...
get_sampling_dist <- function(fit = NULL, dist = NULL) {
  ...
  if (dist == "t") {
    f <- function(n, mu, cov) {
      sigma <- cov*(df - 2)/df
      ch <- try(suppressWarnings(chol(sigma)), silent = TRUE)
      ...
      if (is_error(ch)) {
        ...
        ch <- ch[, order(attr(ch, "pivot")), drop = FALSE]
      }
      mvnfast::rmvt(n, mu = mu, sigma = ch, isChol = TRUE, df = df, kpnames = TRUE)

    }
  }
  else {
    f <- function(n, mu, cov) {
      ch <- try(suppressWarnings(chol(cov)), silent = TRUE)
      #Need pivoted cholesky for when cov isn't PSD (sometimes true for fixed effects models)
      if (is_error(ch)) {
        ch <- suppressWarnings(chol(sigma, pivot = TRUE))
        ch <- ch[, order(attr(ch, "pivot")), drop = FALSE]
      }
      mvnfast::rmvn(n, mu = mu, sigma = ch, isChol = TRUE, kpnames = TRUE)
...
process_coefs <- function(coefs, fit = NULL, coef_supplied) {
  if (coef_supplied == "null") {
    if (is.null(fit)) {
      chk::err("`fit` must be supplied when `coefs` is `NULL`")
    }
    coefs <- get_coefs(fit)
  }
  ...
  if (anyNA(coefs) || any(!is.finite(coefs))) {
    chk::err("the coefficients cannot contain `NA` or non-finite values. This can occur with rank-deficient fits")
  }
...
process_vcov <- function(vcov, fit = NULL, vcov_supplied) {
  if (vcov_supplied == "null") {
    if (is.null(fit)) {
      chk::err("`fit` must be supplied when `vcov` is `NULL`")
    }
    vcov <- get_vcov(fit)
  }
    ...
    if (!check_valid_vcov(vcov)) {
      chk::err("the output of the function supplied to `vcov` must be a square, symmetric, numeric matrix")
    }
  }
  else if (vcov_supplied == "num") {
    if (!check_valid_vcov(vcov)) {
      chk::err("when supplied as a matrix, `vcov` must be a square, symmetric, numeric matrix")
    }
  }

  if (anyNA(vcov) || any(!is.finite(vcov))) {
    chk::err("the covariance matrix cannot contain `NA` or non-finite values. This can occur with rank-deficient fits")
  }
