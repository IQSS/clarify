...
#'
#' @return
#' A `simbased_adrf` object, which inherits from `simbased_est` and is similar to
...
#'
#' @seealso [plot.simbased_adrf()] for plotting the ADRF or AMEF; [sim_ame()] for computing average marginal effects; [sim_apply()], which provides a general interface to computing any
#'   quantities for simulation-based inference; [summary.simbased_est()] for computing
...
#'
#' # ADRF for `age`
#' est <- sim_adrf(s, var = "age", at = seq(15, 55, by = 5),
#'                 verbose = FALSE)
#' est
#' plot(est)
#'
...
sim_adrf <- function(sim,
  ...
  if (contrast == "adrf") {
    ...
    FUN <- function(fit) {
      ...
      vapply(at, function(x) {
        dat[[var]] <- x
        pred <- simbased_predict(fit, newdata = dat, group = outcome, type = type)
  ...
  class(out) <- c("simbased_adrf", class(out))
  out
}

#' @exportS3Method print simbased_adrf
#' @rdname sim_adrf
#' @param x a `simbased_adrf` object.
...
print.simbased_adrf <- function(x, digits = NULL, max.ests = 6, ...) {
  chk::chk_count(max.ests)
  n.ests <- length(attr(x, "original"))
  max.ests <- min(max.ests, n.ests)

  cat("A `simbased_est` object (from `sim_adrf()`)\n")
